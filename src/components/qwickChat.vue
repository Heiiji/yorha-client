...
<template>
  <div v-if="memoire >= 0" class="chatContainer">
    <div @click="showBody = !showBody" class="chatTitle">
      {{conv[memoire][0].sender}}
      <i style="color: white; position: absolute; right: 5px; top: 7px;" class="material-icons">
        clear
      </i>
    </div>
    <div v-if="showBody" class="ChatBody">
        <div id="ChatArea" class="chatArea" v-chat-scroll>
        ...
        AccountServices.GetMSG(this.$store.state.user.local.mail).then((response) => {
          ...
          AccountServices.getMyMsg(vue.$store.state.user.local.mail).then((value) => {
            ...
            for (let i = 0; i < vue.conv.length; i++) {
              if (vue.selectChat) {
                if (vue.selectChat === vue.conv[i][0].senderMail || vue.selectChat === vue.conv[i][0].target) {
                  targ = i
                }
              }
