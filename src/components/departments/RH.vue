...
<template>
  <div :class="'page-inner_' + $store.state.user.local.theme" style="min-height:951px !important">
    <div style="padding: 0px; width: 100%;">
      <div id="main-wrapper">
        <div class="profile-cover" style="background: url('http://blog.prodware.es/wp-content/uploads/2012/05/PLM-y-social-media11.jpg') center; -webkit-background-size: cover; background-size: cover;">
          <div class="dep-title"><div style="background-color: rgba(0, 0, 0, 0.4);">Les ressources humaines</div></div>
        </div>
        ...
        <div class="description-panel" :class="'txt_' + $store.state.user.local.theme">
          <div class="">
            <v-btn v-if="$store.state.user.local.work === 'RH' && $store.state.user.local.qualifier === 'SquadLeader'" @click="NewDescription = description; EditDescription = true;" color="primary" style="float: right;" fab small dark>
              <v-icon>edit</v-icon>
            </v-btn>
  ...
  methods: {
    ...
    GetDep () {
      ...
      this.firebaseApp.auth().currentUser.getIdToken(true).then(function (idToken) {
        DepService.fetchDep(idToken, 'RH').then((response) => {
