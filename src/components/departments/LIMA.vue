...
<template>
  <div>
    <div style="padding: 0px; width: 100%;">
      <div id="main-wrapper">
        <div class="profile-cover" style="background: url('http://www.gotechnologix.com/wp-content/uploads/2016/05/mobile-device-technology-wallpaper.jpg') center; -webkit-background-size: cover; background-size: cover;">
          <div class="dep-title"><div style="background-color: rgba(0, 0, 0, 0.4);">Lima</div></div>
        </div>
        <div class="peoples">
        ...
        <div class="description-panel">
          <div class="">
            <v-btn v-if="$store.state.user.local.work === 'Lima' && $store.state.user.local.qualifier === 'SquadLeader'" @click="NewDescription = description; EditDescription = true;" color="primary" style="float: right;" fab small dark>
              <v-icon>edit</v-icon>
            </v-btn>
  ...
  methods: {
    ...
    GetDep () {
      ...
      this.firebaseApp.auth().currentUser.getIdToken(true).then(function (idToken) {
        DepService.fetchDep(idToken, 'LIMA').then((response) => {
